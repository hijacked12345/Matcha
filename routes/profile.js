...
router.post('/edit', (req, res) => {
    const { fname, lname, username, gender, orientation, tags, bio, distance, lat, lng } = req.body;
    let errors = [];

    if (!fname || !lname || !username || !gender || !orientation || !bio || !tags || !distance || !lat || !lng) {
    ...
    if (errors.length > 0) {
        ...
        User.find({ username: username }).then(user => {
            if (typeof user[0] != 'undefined' && username != req.user.username) {
                ...
                User.updateOne({ username: req.user.username }, { $set: { fname: fname, lname: lname, username: username, gender: gender, orientation: orientation, tags: tagsArray, bio: bio.trim(), distance: distance * 1000, location: { coordinates: [lng, lat] }, } }).then(() => {
                    req.flash(
                        'success_msg',
